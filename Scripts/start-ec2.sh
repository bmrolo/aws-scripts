...
instances=$(aws ec2 describe-instances --filters "Name=instance-state-name,Values=stopped" --query 'Reservations[].Instances[].[Tags[?Key==`Name`].Value | [0], InstanceId, KeyName, PublicIpAddress]' --output json)
...
echo "Available EC2 instances to start:"
...
echo -n "Enter the instance number you want to start: "
